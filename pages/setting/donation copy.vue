...
export default {
      ...
      Paddle.Initialize({
        ...
        eventCallback: async function (data) {
          if (data.name === "checkout.completed") {
            console.log(data);
            try {
              const response = await axios.post('http://localhost:8000/api/paddle-webhook', data)
  ...
  methods: {
    isDisabled() {
      ...
      console.log('store', userasdf.user); // Debugging
    ...
    async submitForm() {
      try {
        const response = await axios.post('http://localhost:8000/api/team-plan-contact', this.contact_form);
